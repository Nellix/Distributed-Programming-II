...
import it.polito.dp2.NFFG.sol3.service.data.ReachabilityPolicyReaderExt;
import it.polito.dp2.NFFG.sol3.service.data.TraversalPolicyReaderExt;
...
public class NffgVerifierImpl implements it.polito.dp2.NFFG.NffgVerifier{
	    ...
	    for(PolicyType p : policies.getReachabilityPolicy())
			try {
				...
				{			
					...
					{
						PolicySet.add(new ReachabilityPolicyReaderExt(p,e));
	    ...
	    for(PolicyType p : policies.getTraversalPolicy())
				try {
					...
					for(NffgReader e : set)
						if(e.getName().compareTo(p.getNffg())==0)
						{
							PolicySet.add(new TraversalPolicyReaderExt(p,e));
						}
					} catch (NffgVerifierException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
	...
	public Set<PolicyReader> getPolicies(String arg0) {
	    ...
	    for(PolicyType p : policies.getReachabilityPolicy())
			try {
				set.add(new ReachabilityPolicyReaderExt(p,nr));
			} catch (NffgVerifierException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	    ...
	    for(PolicyType p : policies.getTraversalPolicy())
				try {
					set.add(new TraversalPolicyReaderExt(p,nr));
				} catch (NffgVerifierException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
