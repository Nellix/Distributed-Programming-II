...
public class NFFGSService {
	...
	public AtomicBoolean getCache() {
		// TODO Auto-generated method stub		
		return NFFGSDB.getCache();
	}


	public void setCache(CachingType value) {
		// TODO Auto-generated method stub
		
		if(value.isStatus())
			NFFGSDB.setCache(true);
		else
			NFFGSDB.setCache(false);
		
	}


	public VerificationResultType testReachabilityMOD(String name) throws NoGraphException, ServiceException {
		// TODO Auto-generated method stub
		
		PolicyType policy = mapPolicy.get(name);

		
		if(policy != null)
		{
			VerificationResultType v = new VerificationResultType();
			v = policy.getVerificationResult();
			NffgType nffg = mapNffg.get(policy.getNffg());
			
			if(v != null)
			{
				if(v.getVerificationTime().compare(nffg.getUpdateTime()) == DatatypeConstants.GREATER)
					return v;
				else
				{
					return testReachabilityPolicy(name);
				}
			}else
			{
